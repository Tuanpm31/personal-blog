---
title: Scheduled job by specify time with flask python
date: 2020-12-08
description: How create cron job and run in specify time with flask server, python
tags:
  - Programing
  - Python
  - Flask
banner: ./scheduled-job-by-specific-time-with-flask-banner.jpeg
---
...
But today we will talk about how to scheduled a job to run in specific time in future with python and flask framework

##  Cron Job with Python
We will use python library [APScheduler](https://apscheduler.readthedocs.io/en/latest/userguide.html) to quick start and easy to scalable way to schedule running task
...
*Create an instance of BackgroundScheduler, and create some func to executed*
```python
...
Very simple, we have two case above, it's trigger by "interval" or "date"
I'll explain them:
1. **Interval** (action will trigger intervals) with many params like:
  ...
  **In example above**, i'm using trigger by interval every day, it will run daily_job, you can try it with hours, seconds, etc...
  You can also specify starting date and ending dates for the schedule through ```start_date``` and ```end_date``` parameters
...
**To add more task, we create route, we can provide body like input time, type of run, but in example, i just create a new normal task when request to /add:
```python
@app.route('/add')
def add_job():
